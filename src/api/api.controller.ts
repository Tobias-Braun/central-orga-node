...
import { TodoList, validateDateString } from '../todo-list/todo-list.entity';
...
export class ApiController {
  ...
  @Get('/eval-today')
  async getDayEvaluation(
    @Query() query: DateStringQuery,
  ): Promise<DayEvaluation> {
    const isValid = validateDateString(query.dateString);
    if (!isValid) {
  ...
  async getPlannedTodos(@Query() query: DateStringQuery): Promise<TodoList> {
    ...
    try {
      const todoList = await this.todoListService.getPlannedForDateString(
        query.dateString,
      );
      return todoList;
    } catch (error) {
