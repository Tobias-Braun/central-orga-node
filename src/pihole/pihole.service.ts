...
export class PiholeService {
    ...
    blockDomains.forEach(async (domain) => {
      try {
        await this.runSafe(domain, async (domain) => {
          const command = this.addToBlockListCommand(domain);
  ...
  async deactivateBlockList(): Promise<ChildProcess[]> {
    ...
    const promises = blockDomains.map(async (domain) =>
      this.runSafe(domain, async (domain) => {
        const command = this.removeFromBlockList(domain);
