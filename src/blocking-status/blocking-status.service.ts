...
export class BlockingStatusService {
  ...
  async getBlockingStatus(): Promise<BlockingStatus> {
    try {
      const blockingStatus = await this.blockingStatusRepository.findOneBy({
        id: 1,
      });
      ...
      this.logger.error(
        "couldn't find blocking status, creating new default one",
      );
  ...
  async updateBlockingStatus(
    ...
    if (blockingStatus.id !== 1) {
      const errorMessage =
        'tried to update an invalid blocking status (id!=1), aborting';
